    ...
    def get(self, request):
        ...
        try:
            ...
            if not user.is_active:
                user.is_active = True
                user.save()
            return redirect('http://watches360.ru/EmailVerification/confirmed')
            # return Response({'email': 'Successfully activated'}, status=status.HTTP_200_OK)
        except jwt.ExpiredSignatureError as identifier:
            return redirect('http://watches360.ru/EmailVerification/expired', {'answer': 'invalid token'})
...
class RequestPasswordResetEmail(generics.GenericAPIView):
    ...
    def post(self, request):
        ...
        if CustomUser.objects.filter(email=email).exists():
            ...
            absurl = 'http://' + current_site + '/NewPassword/' + str(uidb64) + '/' + str(token) + '/'
